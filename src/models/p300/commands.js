...
// Operational commands only — numeric/quality tuning (AGC, EQ/PEQ, AEC NLP, NR level,
// compressor threshold/ratio, automixer attenuation/sensitivity/hold/NOM, direct-out tap)
// is intentionally omitted; this tool is for one-off control commands, not device tuning.
...
export const deviceCommands = [
    ...
    meter('meter_rate_in', 'METER_RATE_IN', 'Inputs', '< SAMPLE_IN aaa bbb ... (14 input levels) >', '0 = off; 1-99 ms invalid. SAMPLE levels 000-060 = -60..0 dBFS. Order: 1-8 Dante mic, 9-10 Dante, 11-12 analog, 13 USB, 14 mobile.'),
    meter('meter_rate_out', 'METER_RATE_OUT', 'Outputs', '< SAMPLE_OUT aaa bbb ... (12 output levels) >', '0 = off. Order: 1-2 Dante out, 3-4 analog out, 5 USB out, 6 mobile out, 7-12 Dante out 3-8.'),
    meter('meter_rate_proc', 'METER_RATE_PROC', 'Processing', '< SAMPLE_PROC aaa bbb ... >', '0 = off. Order: 1-8 pre-AGC Dante inputs, 9 automix out, 10 pre-compressor, 11 AEC reference.'),
    meter('meter_rate_erle', 'METER_RATE_ERLE', 'ERLE', '< SAMPLE_ERLE aaa ... hhh (8 mic channels) >', '0 = off. ERLE data 00-40 dB.'),
    meter('meter_rate_agc', 'METER_RATE_AGC', 'AGC Gain', '< SAMPLE_AGC aaa ... hhh (8 mic channels) >', '0 = off. AGC gain scaled by 20: 00 = -20 dB, 20 = 0 dB, 40 = +20 dB.'),
    dcmd('onhook_enable'),
    dcmd('onhook_state'),
];
